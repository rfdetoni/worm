package com.github.rfdetoni.worm.processor;
 ...
 */
@SupportedAnnotationTypes("com.github.rfdetoni.worm.annotation.mapping.DbTable")
@SupportedSourceVersion(SourceVersion.RELEASE_25)
public final class WormEntityProcessor extends AbstractProcessor {

    private static final String FACTORY_PACKAGE = "com.github.rfdetoni.worm.orm.registry";
    private static final String FACTORY_SUFFIX = "_WormMetadataFactory";
    private static final String BINDER_PACKAGE = "com.github.rfdetoni.worm.orm.mapping";
    ...
    private final Set<String> generatedTypeNamesInRun = new LinkedHashSet<>();
    ...
    private String writeBinderClass(TypeElement entityType) throws IOException {
        ...
        if (!generatedTypeNamesInRun.add(generatedQualifiedName)) {
            return generatedQualifiedName;
        }

        JavaFileObject file;
        try {
            file = filer.createSourceFile(generatedQualifiedName, entityType);
        } catch (FilerException ex) {
            if (isRecreateAttempt(ex)) {
                messager.printMessage(Diagnostic.Kind.NOTE,
                        "Skipping already generated binder: " + generatedQualifiedName, entityType);
                return generatedQualifiedName;
            }
            throw ex;
        }
    ...
    private boolean isBaseEntity(TypeElement entityType) {
        TypeElement iBaseEntity = elements.getTypeElement("com.github.rfdetoni.worm.api.iBaseEntity");
    ...
    private String writeFactoryClass(TypeElement entityType) throws IOException {
        ...
        try {
            ...
            if (isRecreateAttempt(ex)) {
                messager.printMessage(Diagnostic.Kind.NOTE,
                        "Skipping already generated metadata factory: " + generatedQualifiedName, entityType);
                return generatedQualifiedName;
            }
            throw ex;
        }
        try (Writer writer = file.openWriter()) {
            ...
            writer.write("import com.github.rfdetoni.worm.orm.converter.ConverterRegistry;\n");
            writer.write("import com.github.rfdetoni.worm.orm.dialect.SqlDialect;\n");
            writer.write("import com.github.rfdetoni.worm.orm.registry.EntityMetadata;\n");
            writer.write("import com.github.rfdetoni.worm.orm.registry.GeneratedEntityMetadataFactory;\n\n");
            writer.write("/**\n");
            ...
            if (supportsStaticPath) {
                ...
                for (int i = 0; i < joins.size(); i++) {
                    ...
                    writer.write("com.github.rfdetoni.worm.annotation.mapping.DbJoin.Type." + join.joinType);
                    writer.write(")");
    ...
    private void writeServiceFile() throws IOException {
        writeServices("com.github.rfdetoni.worm.orm.registry.GeneratedEntityMetadataFactory", generatedProviderClassNames);
        if (!generatedBinderClassNames.isEmpty()) {
            writeServices("com.github.rfdetoni.worm.orm.mapping.EntityBinder", generatedBinderClassNames);
        }
    }
    ...
    private void writeServices(String serviceType, Set<String> implementations) throws IOException {
        ...
        Set<String> merged = new LinkedHashSet<>(readExistingServices(servicePath));
        merged.addAll(implementations);

        FileObject file = filer.createResource(StandardLocation.CLASS_OUTPUT, "", servicePath);
        try (Writer writer = file.openWriter()) {
            for (String provider : merged) {
    ...
    private Set<String> readExistingServices(String servicePath) {
        Set<String> existing = new LinkedHashSet<>();
        try {
            FileObject existingFile = filer.getResource(StandardLocation.CLASS_OUTPUT, "", servicePath);
            try (BufferedReader reader = new BufferedReader(
                    new InputStreamReader(existingFile.openInputStream(), StandardCharsets.UTF_8))) {
                String line;
            ...
            // No existing service file in this round/location.
        }
        return existing;
    }

    private static boolean isRecreateAttempt(FilerException ex) {
        String msg = ex.getMessage();
        return msg != null && msg.contains("Attempt to recreate a file for type");
    }
