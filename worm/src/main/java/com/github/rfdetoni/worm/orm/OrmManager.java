...
import com.github.rfdetoni.worm.orm.mapping.*;
import com.github.rfdetoni.worm.orm.registry.EntityMetadata;
import com.github.rfdetoni.worm.orm.registry.EntityRegistry;
import com.github.rfdetoni.worm.orm.registry.ProjectionMetadata;
import com.github.rfdetoni.worm.orm.sql.*;
import com.github.rfdetoni.worm.orm.tracking.EntitySnapshot;
import com.github.rfdetoni.worm.orm.tracking.SessionSnapshotContext;
...
public class OrmManager implements OrmOperations {
    ...
    private <T> void assertOptimisticLock(EntityMetadata<T> metadata, T entity, Object id, int rows) {
        ...
        throw new OptimisticLockException(metadata.entityClass(), id, version);
    }
    ...
    private <T> EntitySnapshot getSnapshot(T entity) {
        ...
        if (entity instanceof ActiveRecord<?, ?> activeRecord) {
            ...
            int[] results = execWrite(() -> {
                ...
                if (meta.hasVersion()) {
                    for (int i = 0; i < batchResults.length; i++) {
                        if (batchResults[i] == 0) {
                            throw new OptimisticLockException(meta.entityClass(), ids.get(i), versions.get(i));
                        }
                    }
                }
                return batchResults;
    ...
    public <T, P> List<P> findAll(Class<T> entityClass, FilterBuilder filter, Class<P> projectionClass) {
        ...
        return withModule(metadata, () -> {
            final ProjectionMetadata proj = EntityRegistry.getProjectionMetadata(projectionClass, metadata);
            final QueryBuilder<T> qb = new QueryBuilder<>(metadata, filter, dialect);
    ...
    private <P> List<P> aggregateProjectionRows(List<P> rows, ProjectionMetadata proj) {
