...
import com.github.rfdetoni.worm.annotation.audit.*;
import com.github.rfdetoni.worm.annotation.mapping.*;
...
import com.github.rfdetoni.worm.orm.mapping.ColumnConverter;
import com.github.rfdetoni.worm.orm.mapping.ParamBinder;
import com.github.rfdetoni.worm.orm.query.JoinClauseBuilder;
...
final class MetadataBuilder<T> {
    ...
    private static List<Field> getAllFields(Class<?> clazz) {
        ...
        while (current != null && current != Object.class) {
            for (Field f : current.getDeclaredFields()) {
                ...
                if (f.getDeclaringClass() == ActiveRecord.class) continue;
                if ((Modifier.isTransient(mods) || Modifier.isFinal(mods))
                        && Arrays.stream(f.getAnnotations()).noneMatch(a -> a.annotationType().getName().startsWith("com.github.rfdetoni.worm.annotation."))) continue;
    ...
    private void applyJoinAnnotation(JoinInfo ji, DbJoin ann, String relationName, boolean isCollection) {
        ...
        ji.type = ann.type();
        ji.fetchMode = ann.fetchMode();
        // For BATCH-fetch collection joins, store the FK column on the child side
        // so BatchFetchExecutor can build IN-clause queries without re-parsing the ON string.
        if (ji.fetchMode == DbJoin.FetchMode.BATCH && isCollection) {
            ji.mappedByColumn = resolveMappedByColumn(ann, ji, isCollection);
        }
    }
    ...
    private EntityMetadata<T> assembleFinal(Field[] allFields)
        ...
        boolean tracked = entityClass.isAnnotationPresent(Track.class);
        ...
        if (isRecord && constructorHandle != null && paramLabels.size() > 0) {
            try {
                constructorSpreader = constructorHandle.asSpreader(Object[].class, paramLabels.size());
            } catch (IllegalArgumentException ignored) {
                constructorSpreader = null;
            }
        }

        MethodHandle createdHook = buildLifecycleHook("created");
        MethodHandle updatedHook = buildLifecycleHook("updated");

        WritePlan insertWritePlan = buildInsertWritePlan(
                insertSql,
                insertable,
                createdAtCol,
                updatedAtCol,
        ...
        WritePlan updateWritePlan = buildUpdateWritePlan(
                updateSql,
                effectiveUpdatable,
                updatedAtCol,
                colIndex,
    ...
    private MethodHandle buildLifecycleHook(String methodName) {
        if (!iBaseEntity.class.isAssignableFrom(entityClass)) return null;
        try {
            return lookup.findVirtual(iBaseEntity.class, methodName, MethodType.methodType(void.class));
        } catch (NoSuchMethodException | IllegalAccessException e) {
            throw new RuntimeException("Failed to compile iBaseEntity." + methodName + " hook for " + entityClass.getName(), e);
        }
    }

    private WritePlan buildInsertWritePlan(String insertSql,
                                           List<String> insertableColumns,
                                           Optional<String> createdAtCol,
                                           Optional<String> updatedAtCol,
                                           boolean hasActive,
                                           String activeCol,
        ...
        for (int i = 0; i < insertableColumns.size(); i++) {
            ...
            if (createdAtCol.isPresent() && createdAtCol.get().equals(column)) {
                slots[i] = new WritePlan.Slot.AuditNow(isLocalDateTimeColumn(columnIndex, types, column));
                continue;
            }
            ...
            int idx = requiredColumnIndex(columnIndex, column);
            MethodHandle getter = getters.get(idx);
            Class<?> type = types.get(idx);
            if (hasActive && activeCol != null && activeCol.equals(column)) {
                slots[i] = new WritePlan.Slot.ActiveDefault(getter, activeDefaultValue, type == boolean.class);
            } else {
                slots[i] = new WritePlan.Slot.Field(getter, shouldSerializeAsJson(column, type), type.isEnum());
            }
        }
        return createWritePlan(insertSql, slots, hookHandle, false);
    }

    private WritePlan buildUpdateWritePlan(String updateSql,
                                           ...
                                           boolean hasVersion,
                                           MethodHandle hookHandle) {
        int slotCount = updatableColumns.size() + 1 + (hasVersion ? 1 : 0);
        WritePlan.Slot[] slots = new WritePlan.Slot[slotCount];
        int out = 0;
        for (String column : updatableColumns) {
            if (updatedAtCol.isPresent() && updatedAtCol.get().equals(column)) {
                slots[out++] = new WritePlan.Slot.AuditNow(isLocalDateTimeColumn(columnIndex, types, column));
                continue;
            }
            int idx = requiredColumnIndex(columnIndex, column);
            Class<?> type = types.get(idx);
            slots[out++] = new WritePlan.Slot.Field(getters.get(idx), shouldSerializeAsJson(column, type), type.isEnum());
        }
        slots[out++] = new WritePlan.Slot.Field(idGetter, false, false);
        if (hasVersion) {
            slots[out] = new WritePlan.Slot.Field(versionGetter, false, false);
        }
        return createWritePlan(updateSql, slots, hookHandle, hasVersion);
    }
    ...
    private int requiredColumnIndex(Map<String, Integer> columnIndex, String column) {
        Integer idx = columnIndex.get(column);
        if (idx == null) {
            throw new IllegalStateException("No column index found for write-plan column '" + column + "' on " + entityClass.getName());
        }
        return idx;
    }

    private boolean isLocalDateTimeColumn(Map<String, Integer> columnIndex, List<Class<?>> types, String column) {
        int idx = requiredColumnIndex(columnIndex, column);
        return types.get(idx) == LocalDateTime.class;
    }

    private boolean shouldSerializeAsJson(String column, Class<?> type) {
        return jsonCols.contains(column) || isJsonCandidate(type);
    }
    ...
    private String resolveMappedByColumn(DbJoin ann, JoinInfo ji, boolean isCollection) {
        // Explicit mappedBy wins
        if (!ann.mappedBy().isBlank()) return ann.mappedBy();
        // Parse the ON clause: alias.fk_col = mainAlias.pk → take the left side's column
        if (!ann.on().isBlank()) {
            String on = ann.on().trim();
            int eq = on.indexOf('=');
            if (eq > 0) {
                String left = on.substring(0, eq).trim();
                int dot = left.lastIndexOf('.');
                if (dot >= 0) return left.substring(dot + 1).trim();
            }
        }
        // Infer from joinInfo ON clause (already resolved)
        if (ji.on != null && !ji.on.isBlank()) {
            String on = ji.on.trim();
            int eq = on.indexOf('=');
            if (eq > 0) {
                String left = on.substring(0, eq).trim();
                int dot = left.lastIndexOf('.');
