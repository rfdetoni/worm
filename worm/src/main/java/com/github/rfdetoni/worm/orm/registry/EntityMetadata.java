...
import com.github.rfdetoni.worm.orm.mapping.EntityBinder;
...
public final class EntityMetadata<T> {
    ...
    /**
     * Pre-compiled spreader handle ({@code constructor.asSpreader(Object[].class, paramCount)})
     * for zero-copy record construction.  {@code null} for POJO entities or when the
     * spreader cannot be compiled (vararg constructors, etc.).
     */
    private final MethodHandle constructorSpreader;
    private final WritePlan insertWritePlan;
    private final WritePlan updateWritePlan;
    private final EntityBinder<T> binder;
    ...
    EntityMetadata(Builder<T> b) {
        ...
        this.constructorSpreader = b.constructorSpreader;
        this.insertWritePlan    = b.insertWritePlan;
    ...
    /**
     * Pre-compiled spreader handle for zero-copy record construction.
     * May be {@code null} — callers must fall back to {@link #constructor()}.{@code invokeWithArguments}.
     */
    public MethodHandle      constructorSpreader() { return constructorSpreader; }
    public WritePlan         insertWritePlan()     { return insertWritePlan; }
    ...
    /**
     * Returns {@code true} if any collection join is configured with
     * {@link DbJoin.FetchMode#BATCH}.
     * When true, {@code OrmManager.findAll} uses {@code BatchFetchExecutor}
     * instead of the standard Cartesian-join + merge path.
     */
    public boolean hasBatchFetchJoins() {
        if (joinInfos == null) return false;
        for (JoinInfo ji : joinInfos) {
            if (ji != null && ji.isList() && ji.getFetchMode() == DbJoin.FetchMode.BATCH) return true;
        }
        return false;
    }
    ...
    static final class Builder<T> {
        ...
        /** Pre-compiled spreader — null for POJOs or when compilation fails. */
        MethodHandle constructorSpreader;
        WritePlan insertWritePlan, updateWritePlan;
        EntityBinder<T> binder;
