...
public final class JoinInfo {
    ...
    String on;
    DbJoin.Type type;
    /** Fetch strategy for collection joins (LIST fields). Defaults to JOIN. */
    DbJoin.FetchMode fetchMode = DbJoin.FetchMode.JOIN;
    /** FK column on the joined (child) table — populated for BATCH-fetch collection joins. */
    String mappedByColumn;
    ...
    /**
     * Pre-compiled spreader handle for zero-copy record join construction.
     * {@code null} for POJO joins or when compilation fails.
     */
    MethodHandle joinConstructorSpreader;
    ...
    JoinInfo(Class<?> joinClass, boolean isRecord, MethodHandle joinConstructor,
        ...
        // Pre-compile spreader for record joins
        if (isRecord && joinConstructor != null && !joinColumnNames.isEmpty()) {
            try {
                this.joinConstructorSpreader = joinConstructor.asSpreader(Object[].class, joinColumnNames.size());
            } catch (IllegalArgumentException ignored) {
                this.joinConstructorSpreader = null;
            }
        }
    }
    ...
    public DbJoin.FetchMode getFetchMode()               { return fetchMode; }
    public String           getMappedByColumn()          { return mappedByColumn; }
    ...
    public MethodHandle     getJoinConstructorSpreader() { return joinConstructorSpreader; }
