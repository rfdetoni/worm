...
public final class GeneratedMetadataRuntimeSupport {
    ...
    private static JoinInfo inspectJoin(JoinDescriptor descriptor, MethodHandles.Lookup rootLookup) throws Exception {
        ...
        if (joinIsRecord) {
            ...
            for (int i = 0; i < components.length; i++) {
                var rc = components[i];
                String col = resolveJoinColumnName(rc.getName(), rc.getAccessor().getAnnotation(DbColumn.class));
                colNames.add(col);
        ...
        for (Field field : joinClass.getDeclaredFields()) {
            ...
            field.setAccessible(true);
            String col = resolveJoinColumnName(field.getName(), field.getAnnotation(DbColumn.class));
            colNames.add(col);
    ...
    private static JoinInfo inspectJoinColumns(Class<?> joinClass) {
        try {
            ...
            String table = JOIN_TABLE_CACHE.computeIfAbsent(joinClass, jc -> {
                var dbTable = jc.getAnnotation(DbTable.class);
                if (dbTable != null && !dbTable.value().isBlank()) return dbTable.value();
                return jc.getSimpleName().toLowerCase();
            });
            return inspectJoin(new JoinDescriptor("", joinClass, table, AliasUtils.defaultMainAlias(table), "", DbJoin.Type.INNER), MethodHandles.lookup());
        } catch (Exception e) {
    ...
    private static String resolveJoinColumnName(String defaultName, DbColumn dbColumn) {
