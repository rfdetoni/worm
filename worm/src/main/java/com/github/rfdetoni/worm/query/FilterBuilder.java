...
import com.github.rfdetoni.worm.orm.registry.WormAttribute;
...
public final class FilterBuilder {
    ...
    public FilterBuilder join(JoinType type, String table, String alias, String on) {
        ...
        if (!baseAlias.equals(useAlias)) {
            adjustedOn = replaceQualifiedPrefixIgnoreCase(on, baseAlias, useAlias);
        }
    ...
    // ── WormAttribute typed overloads (compile-time column safety) ───────────

    /** Typed {@code =} predicate using a {@link WormAttribute} column descriptor. */
    public <V> FilterBuilder eq(WormAttribute<?, V> attr, V value)          { return eq(attr.columnName(), value); }
    /** Typed {@code <>} predicate. */
    public <V> FilterBuilder neq(WormAttribute<?, V> attr, V value)         { return neq(attr.columnName(), value); }
    /** Typed {@code >} predicate. */
    public <V> FilterBuilder gt(WormAttribute<?, V> attr, V value)          { return gt(attr.columnName(), value); }
    /** Typed {@code <} predicate. */
    public <V> FilterBuilder lt(WormAttribute<?, V> attr, V value)          { return lt(attr.columnName(), value); }
    /** Typed {@code >=} predicate. */
    public <V> FilterBuilder gte(WormAttribute<?, V> attr, V value)         { return gte(attr.columnName(), value); }
    /** Typed {@code <=} predicate. */
    public <V> FilterBuilder lte(WormAttribute<?, V> attr, V value)         { return lte(attr.columnName(), value); }
    /** Typed {@code LIKE} predicate. */
    public FilterBuilder like(WormAttribute<?, String> attr, String value)   { return like(attr.columnName(), value); }
    /** Typed {@code IN} predicate. */
    public <V> FilterBuilder in(WormAttribute<?, V> attr, Collection<V> values) { return in(attr.columnName(), values); }
    /** Typed {@code IS NULL} predicate. */
    public FilterBuilder isNull(WormAttribute<?, ?> attr)                    { return isNull(attr.columnName()); }
    /** Typed {@code IS NOT NULL} predicate. */
    public FilterBuilder isNotNull(WormAttribute<?, ?> attr)                 { return isNotNull(attr.columnName()); }
    /** Typed ascending ORDER BY. */
    public FilterBuilder orderBy(WormAttribute<?, ?> attr)                   { return orderBy(attr.columnName()); }
    /** Typed directional ORDER BY. */
    public FilterBuilder orderBy(WormAttribute<?, ?> attr, boolean ascending) { return orderBy(attr.columnName(), ascending); }
    /** Typed descending ORDER BY. */
    public FilterBuilder orderByDesc(WormAttribute<?, ?> attr)               { return orderByDesc(attr.columnName()); }
    ...
    /**
     * Appends a raw pre-built WHERE clause (already containing {@code ?} placeholders)
     * together with its matching parameter list. Intended for internal use when copying
     * a FilterBuilder's WHERE state without rebuilding every predicate.
     *
     * @param rawClause raw clause string, e.g. {@code "status = ? AND active = true"}
     * @param params    ordered bind values matching the {@code ?} placeholders
     * @return this builder
     */
    public FilterBuilder rawWhere(String rawClause, java.util.List<Object> params) {
        if (rawClause == null || rawClause.isBlank()) return this;
        and();
        whereClause.append(rawClause);
    ...
    private void replaceMainAliasOccurrences(String fromAlias, String toAlias) {
        ...
        if (!where.isEmpty()) {
            String replaced = replaceQualifiedPrefixIgnoreCase(where, fromAlias, toAlias);
        ...
        for (int i = 0; i < joins.size(); i++) {
            Join j = joins.get(i);
            String on = j.on();
            String newOn = on == null ? null : replaceQualifiedPrefixIgnoreCase(on, fromAlias, toAlias);
