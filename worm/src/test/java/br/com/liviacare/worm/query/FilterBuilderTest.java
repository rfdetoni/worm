...
class FilterBuilderTest {
    ...
    @Test
    void conflictingAlias_rewritesOnlyQualifiedAliasToken() {
        FilterBuilder f = FilterBuilder.where()
                .leftJoin("orders", "o", "oo.user_id = a.id AND o.user_id = a.id")
                .leftJoin("items",  "o", "o.order_id = x.id");

        assertEquals("oo.user_id = a.id AND o.user_id = a.id", f.getJoins().get(0).on());
        assertEquals("o2.order_id = x.id", f.getJoins().get(1).on());
    }
    ...
    @Test
    void aliasReplacement_doesNotMutateLongerIdentifiers() {
        FilterBuilder f = FilterBuilder.where()
                .eq("a.status", "ACTIVE")
                .leftJoin("orders", "o", "O.user_id = A.id AND aa.id = o.id")
                .alias("u");

        assertEquals("u.status = ?", f.getWhereClause());
        assertEquals("O.user_id = u.id AND aa.id = o.id", f.getJoins().get(0).on());
    }
